...
main = do
    [n,m] <- list1 ucInt
    graph <- genGraph (1,n) . fmap to2 <$> list2 m ucInt

    seen <- fmap (V.group . vSort . V.fromList) $ (dfs graph [1..n] :: IO (IOUArray Int Int)) >>= getElems
    
    let results = map (\vec -> let x = V.length vec
                               in ((x * (x-1)) `div` 2)) seen
    print $ sum results - m

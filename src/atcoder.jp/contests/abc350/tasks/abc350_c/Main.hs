...
main = do
    n <- readLn @Int
    as <-  V.fromListN n . zip [1..] <$> list1 ucInt

    let bs = fst <$> vSortBy (\x y -> compare (snd x) (snd y)) as

    as1 <- V.thaw $ snd <$> as
    bs1 <- V.thaw bs


    result <- fmap (filter (uncurry (/=))) $ forM [1..n-1] $ \i -> do
        a <- VM.read as1 (i-1)
        b <- VM.read bs1 (i-1)
        VM.write as1 (i-1) (i-1)
        VM.write as1 (b-1) a
        VM.write bs1 (i-1) (i-1)
        VM.write bs1 (a-1) b
        return (i, b)

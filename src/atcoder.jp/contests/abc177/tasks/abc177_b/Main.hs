...
main = do
  ...
  ts <- readLnAsListWith unconsChar

  print $ minimum $ solve (VU.fromList ss) (VU.fromList ts)

solve :: VU.Vector Char -> VU.Vector Char -> [Int]
solve svec tvec
  | VU.length svec < VU.length tvec = []
  | otherwise = go : solve (VU.tail svec) tvec
  where go = VU.length $ VU.filter (\(s,t) -> s /= t) $ VU.zip svec tvec

...
main = do
    ...
    ps <- list1 ucInt
    q <- readLn @Int
    lrs <- fmap to2 <$> list2 q ucInt
    let xps = zip xs ps
        cumxps = V.scanl (\(_,p1) (x2,p2) -> (x2, p1+p2)) (-10^9-1, 0) $ V.fromList xps

    forM_ lrs $ \(l,r) -> do
        let il = bsearch (\i -> fst (cumxps V.! i) >= l) (n+1) (-1)
            ir = bsearch (\i -> fst (cumxps V.! i) <= r) (-1) (n+1)

        print $ snd (cumxps V.! ir) - snd (cumxps V.! (il-1))

...
main = do
    [h,w] <- list1 ucInt
    grid <- toCharGrid ((1,1),(h,w)) <$> list2 h ucChar

    let b = bounds grid
    seen <- newArray @IOArray b Nothing

    let aroundMag = any (\v -> grid ! v == '#') . arounds b nei4
        isMag v = grid ! v == '#'


    results <- forM [(i,j) | i<-[1..h], j<-[1..w], not (isMag (i,j))] $ dfs aroundMag (filter (\v -> grid ! v /= '#') . arounds b nei4) b seen
...
dfs :: forall a m i r. (MArray a (Maybe i) m, Ix i, Ref r m, Show i) =>
    (i -> Bool) ->
    -- | 現在点から探索候補点を取得
    (i -> [i]) ->
    -- | 探索範囲のbound
    ...
    i ->
    m Int
dfs cond nexts b seen start = do
    ref <- newRef 0
    go ref seen start
    readRef ref
    where
        go :: r Int -> a i (Maybe i) -> i -> m ()
        go ref seen v = do
            s <- readArray seen v
            if cond v
                then do
                case s of
                    Nothing -> do
                        ...
                        modifyRef' ref (1+)
                    Just u -> if u == start then return () else do
                        writeArray seen v (Just start)
                        modifyRef' ref (1+)
                else do
                case s of

...
main = do
  ...
  edges <- readLnAsUVecWith2Tuple unconsInt (n-1)
  queries <- readLnAsUVecWith2Tuple unconsInt q
  let g = graph edges queries n
      ans = unwords . V.toList $ V.map (show . fst) g
...
graph :: (VU.Unbox a, Num a) => VU.Vector (Vertex, Vertex) -> VU.Vector (Vertex, a) -> Int -> Graph a
graph edge query n = V.create $ do
  g <- VM.replicate n (0,[])

  -- グラフの構築
  let le = VU.length edge
  forM_ [0..le-1] $ \i -> do
    let (from,to) = edge VU.! i
        f = from - 1
        t = to - 1
    VM.modify g (\(w,ns)->(w,t:ns)) f
    VM.modify g (\(w,ns)->(w,f:ns)) t

  -- 各頂点に値を追加
  let lq = VU.length query
  forM_ [0..lq-1] $ \i -> do
    let (p,x) = query VU.! i
    VM.modify g (\(w,ns)->(w+x,ns)) (p-1)

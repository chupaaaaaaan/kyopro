...
main = do
    n <- readLn @Int
    as <- rowv @V.Vector n unconsInt
    q <- readLn @Int
    bs <- colv @V.Vector q unconsInt
    let asSorted = sortedVector as

    V.forM_ bs $ \b -> print $ solve n asSorted q b

solve :: Int -> V.Vector Int -> Int -> Int -> Int
solve n as q b =
    let idx_g = binarySearch (isOkGreater as b) (n -1) 0
        idx_l = binarySearch (isOkLesser as b) 0 (n -1)
        val_g = as V.! idx_g
        val_l = as V.! idx_l
     in min (abs (val_g - b)) (abs (b - val_l))

isOkGreater, isOkLesser :: V.Vector Int -> Int -> Int -> Bool
isOkGreater v key idx = v V.! idx >= key
isOkLesser v key idx = v V.! idx <= key
...
-- Vector
sortedVector :: (Ord a, Vector v a) => v a -> v a
sortedVector v = runST $ do
    mv <- VG.thaw v
    sort mv
    VG.freeze mv

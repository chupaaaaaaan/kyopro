...
main = do
    [n,k] <- list1 ucInt
    abv <- toVect <$> list2 n ucInt

    print $ solve n k abv


solve :: Int -> Int -> Vector (Int, Int) -> Int
solve n k abv = runST $ do
    v <- V.thaw abv
    sortBy (\(a,b) (c,d) -> a `compare` c) v
    v' <- V.freeze v

    let bsum = V.sum $ V.map snd v'
        q = V.scanl' (\(a,b) (c,d) -> (c+1, b-d)) (1, bsum) v'

    return $ fst $ V.head $ V.dropWhile (\(x,y) -> y > k) q

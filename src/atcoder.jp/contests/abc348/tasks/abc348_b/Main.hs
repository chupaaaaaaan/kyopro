...
main = do
    n <- readLn @Int
    ixys <- zip [1..] . map (\[x,y] -> (x,y)) <$> list2 n ucInt

    putStr $ unlines $ map show $ solve n ixys


solve :: Int -> [(Int, (Int,Int))] -> [Int]
solve n ixys = [go ixy ixys | ixy <- ixys ]
    where go :: (Int, (Int,Int)) -> [(Int, (Int,Int))] -> Int
          go (i,(xi,yi)) jxys = fst $ maximumBy (\p q -> let sc = compare (snd p) (snd q)
                                                         in if sc /= EQ then sc else compare (fst q) (fst p)) $
                                map (\(j,(xj,yj)) -> if i == j then (j, 0) else (j, (xj-xi)^2 + (yj-yi)^2)) jxys

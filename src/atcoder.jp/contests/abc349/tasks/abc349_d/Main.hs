...
main = do
    ...
    print m
    putStr $ unlines $ map (\(x,y) -> show x <> " " <> show y) $ reverse as
    ...
    where go :: Int -> [(Int,Int)] -> Int -> (Int, [(Int,Int)])
          go m as l'
              ...
              | otherwise =
                    let next = head
                            $ dropWhile (\x -> snd x > r)
                            $ L.sortBy (\x y -> compare (snd y) (snd x))
                            $ map ((\(i,j) -> (2^i*j, 2^i*(j+1))) . (\i -> (i,l'`div`2^i)))
                            $ takeWhile (\i -> l'`mod`(2^i)==0 && l' + 2^i <= r) [0..]
                    in go (m+1) (next:as) $ snd next

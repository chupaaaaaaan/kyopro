...
main = do
    ...
    putStr $ unwords . map show $ solve n graph
    putStrLn ""

    return ()

solve :: Int -> WGraph Int Int -> [Int]
solve n graph = let curr = traceWithPrefix "curr: " $ runSTUArray $ snd <$> dijkstra (traceWithPrefix "graph: " graph) 1
                    go as k d | k == 1 && d == 0 = Just as
                              | d < 0 = Nothing
                              | otherwise = let nexts = filter (\x -> curr ! k > curr ! fst x) $ graph ! k
                                            in if null nexts then Nothing else msum $ map (\(m, w) -> go (m:as) m (d - w)) nexts
                in fromMaybe [] $ go [n] n (curr ! n)

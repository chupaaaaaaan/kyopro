...
main = do
  n <- readLn @Int
  sts <- concatMap words <$>  replicateM n getLine
...
solve [] = "Yes"
solve [_] = error "error"
solve (x:y:xs) = case (x `notElem` xs, y `notElem` xs) of
                        (True, True) -> solve xs
                        (True, False) -> solve (xs <> [y])
                        (False, True) -> solve (xs <> [x])
                        (False, False) -> "No"
...
unconsString :: StateT BS.ByteString Maybe BS.ByteString
unconsString = StateT $ Just . BS.span isSpace

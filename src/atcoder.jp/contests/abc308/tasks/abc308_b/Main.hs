...
main = do
    [n,m] <- list1 ucInt
    cs <- fmap BS.unpack . BS.words  <$> BS.getLine
    ds <- fmap BS.unpack . BS.words  <$> BS.getLine
    (p0:ps) <- list1 ucInt



    print $ solve n m cs ds (p0:ps)
...
solve n m cs ds (p0:ps) = go 0 cs
    where go :: Int -> [String] -> Int
          go a [] = a
          go a (c:cs) = case L.lookup c dps of
                            Nothing -> go (a + p0) cs
                            Just p -> go (a + p) cs
          dps = zip ds ps

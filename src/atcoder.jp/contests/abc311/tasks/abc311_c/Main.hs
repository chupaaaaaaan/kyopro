...
main = do
    n <- readLn @Int
    graph <- listArray @UArray (1,n) <$> list1 ucInt
    
    let (m, as) = solve n graph
    print m
    putStrLn $ unwords $ map show $ reverse as

solve :: Int -> UArray Int Int -> (Int, [Int])
solve n arr = let precycle = ho 0 1
              in go precycle [] 0 precycle
    where go :: Int -> [Int] -> Int -> Int -> (Int, [Int])
          go i as m k | i == k && m /= 0 = (m, as)
                      | otherwise = go i (k:as) (m+1) (arr ! k)
          ho :: Int -> Int -> Int
          ho a i | a == n = i
                 | otherwise = ho (a+1) (arr ! i)

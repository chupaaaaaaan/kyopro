...
main = do
    n <- readLn @Int
    as <- reverse . concat <$> list2 n ucString

    forM_ as BS.putStrLn
...
ucString = StateT (\bs -> let bs' = BS.dropWhile isSpace bs in if BS.null bs' then Nothing else Just $ BS.break isSpace bs')

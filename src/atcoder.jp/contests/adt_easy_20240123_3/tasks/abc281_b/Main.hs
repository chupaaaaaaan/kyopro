...
main = do
    ...
    if BS.length as /= 8 then putStrLn "No" else do
        let Just (b,bs) = BS.uncons as
            Just (cs,d) = BS.unsnoc bs

        putStrLn $ if isUpper b && BS.all isNumber cs && BS.head cs /= '0' && isUpper d
                   then "Yes"
                   else "No"

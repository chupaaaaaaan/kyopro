...
main :: IO ()
main = do
    [ax,ay,bx,by,cx,cy,dx,dy] <- concat <$> list2 4 ucInt

    let f x y = (cy - ay) * (x - ax) - (cx - ax) * (y - ay)
        g x y = (dy - by) * (x - bx) - (dx - bx) * (y - by)

    putStrLn $ if f bx by * f dx dy < 0 && g ax ay * g cx cy < 0 then "Yes" else "No"

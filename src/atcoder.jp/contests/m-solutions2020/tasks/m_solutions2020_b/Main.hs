...
main :: IO ()
main = do
  [a,b,c] <- readLnAsListWith unconsInt
  k <- readLn
  putStrLn $ if solve k a b c then "Yes" else "No"
...
solve 0 a b c = a < b && b < c
solve k a b c = let b' = b * 2
                    c' = c * 2
                in if (a < b' && b' < c) || (a < b && b < c')
                   then True
                   else solve (k-1) a b' c || solve (k-1) a b c'

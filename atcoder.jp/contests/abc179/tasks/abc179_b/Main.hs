...
main = do
  n <- readLn @Int
  xyvec <- VU.map (uncurry (==)) <$> readLnAsUVecWith2Tuple unconsInt n

  putStrLn $ if solve n xyvec then "Yes" else "No"


solve :: Int -> VU.Vector Bool -> Bool
solve n xyvec = go 0 0
  where go m i
          | m >= 3 = True
          | m < 3 && i == n =  False
          | otherwise = if xyvec VU.! i
                        then go (m+1) (i+1)
                        else go 0 (i+1)

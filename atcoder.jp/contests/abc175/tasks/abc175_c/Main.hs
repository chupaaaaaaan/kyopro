...
main :: IO ()
main = do
  [x,k,d] <- readLnAsListWith unconsInt
  let stepToMin = abs x `div` d
      restStep = k - stepToMin
  print $ if | abs x > d && restStep < 0 -> abs x - k * d
             | abs x > d && restStep >= 0 && even restStep -> abs x `mod` d
             | abs x > d && restStep >= 0 && odd restStep -> d - abs x `mod` d
             | abs x == d && even k -> d
             | abs x == d && odd k -> 0
             | abs x < d && even k -> abs x
             | abs x < d && odd k -> d - abs x
